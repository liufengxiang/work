<?php
    class code{
        ...
        public $fontSize=Array("min"=>16,"max"=>20);    //文字大小
        public $fontAngle=10;   //文字旋转角度
        public $resultLetter="";
        public $img;
        /*
         * 创建图片并输出资源
        ...
        public function getText(){
            $nub=$this->letterNum;
            for($i=0;$i<$nub;$i++){
                $this->resultLetter.=$this->letter[rand(0,strlen($this->letter)-1)];
            }
        }
        /*
         * 在图片资源上生成文字，随机颜色
        ...
        private function createText(){
            ...
            for($i=0;$i<$this->letterNum;$i++){
                ...
                $angle=$reg=rand(-$this->fontAngle,$this->fontAngle);
                $str=$this->resultLetter[$i];
                $rect=imagettfbbox($fontSize,$angle,$this->letterUrl,$str);
                $lx=5+$marginX*$i;
                $ly=rand(($rect[1]-$rect[5]),($this->height-5));
        ...
        private function createPx(){
            $nub=rand(30,50);
            while($nub--){
                $lx=rand(0,$this->width);
                $ly=rand(0,$this->height);
        ...
        function randColor($bottom=50,$top=200){
            $arr[0]=rand($bottom,$top);
            $arr[1]=rand($bottom,$top);
            $arr[2]=rand($bottom,$top);
