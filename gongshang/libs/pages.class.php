...
class pages{
    public $total;//数据总量
    public $pageNum;//每页显示的条数
    public $current;//当前的页数
    public $pageNums;//总页数
    public $param;//地址栏默认的参数
    public $limit;
    function __construct($total,$pageNum=5,$param="pages"){
        $this->total=$total;
    ...
    function out(){
        ...
        $str.="<span>".$this->pageNums."</span>";//一共有**页
        $str.="<a href='{$url}0' id='home'>首页</a>";
        $up=$this->current-1<0?0:$this->current-1;
        $str.="<a href='{$url}{$up}' id='up'>上一页</a>";
        ...
        for($i=$start;$i<$end;$i++){
            ...
            $str.="<a href='{$url}{$i}' class='{$class}' id='p$i'>".$n."</a>";
        }
        $next=$this->current+1>$this->pageNums-1?$this->pageNums-1:$this->current+1;
        $str.="<a href='{$url}{$next}' id='next'>下一页</a>";
        $last=$this->pageNums-1;
        $str.="<a href='{$url}{$last}' id='last'>尾页</a>";
